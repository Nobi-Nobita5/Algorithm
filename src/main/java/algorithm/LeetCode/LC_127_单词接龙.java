 ...
 * 思路：bfs+优化建图，来自题解，用户：liweiwei1419
 *
...
public class LC_127_单词接龙 {
     ...
     *
     * 1.如果一开始就构建图，每一个单词都需要和除它以外的另外的单词进行比较，复杂度是 O(N×wordLen)，这里 N 是单词列表的长度；
     * 2.为此，我们一开始把所有的单词列表放进一个哈希表中，然后在遍历的时候构建图，每一次得到在单词列表里可以转换的所有单词，复杂度是 O(26×wordLen)，
     *   借助哈希表，找到邻居与 N 无关；
     * 3.使用 BFS 进行遍历，需要的辅助数据结构是：
     *   1)队列；
