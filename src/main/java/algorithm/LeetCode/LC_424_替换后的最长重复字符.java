 ...
 *
 * 实际代码中，由于【字符串中仅包含大写字母】，我们可以使用一个长度为 26 的数组维护每一个字符的出现次数。每次区间右移，我们更新右移位置的字符出现的次数，
 ...
 *                     同时更新字符出现次数的最大值，最后根据【当前窗口长度】-【字符出现次数最大值】 与 k比较，
    ...
    public int characterReplacement(String s, int k) {
        ...
        while (right < n) {
            ...
            if (right - left + 1 - maxn > k) {
                ...
                num[s.charAt(left) - 'A']--;
                left++;
                //右窗口也同步右移了。right - left 保持不变。
                //1. 如果左指针右移之后的窗口能再增大右边界以产生更长的满足要求的子串，则继续扩大右边界。
                //2. 如果左指针右移之后的窗口不能再增大右边界以产生更长的满足要求的子串，还是可以继续将整个窗口右移（right - left 保持不变），继续寻找更优的答案。
                //3. 左指针右移后，不能改变maxn的值，因为我们需要等待，直至更新右移位置字符的出现次数 比 maxn 更大时，就代表寻找到了更优的答案。
            }
            right++;
        }
        return right - left;//不断更新窗口的长度right - left，得到最终的答案
    }
}
