 ...
 *
 * -------------------------------------------------
 * [1,2,3,4]:根据权重划分0-10区间，[1,1],[2,3],[4,6],[7,10],返回4的概率是40%。前缀和数组可以表示出这种权重。
 *
 * 思路：
 * 用一个数组表示数组w[i]的前缀和:sum[i] = {1,3,6,10}。
 * 那么我们可以在{1-sum[w.length-1]}之间生成随机数x。然后使用二分查找，找到满足sum[i] >= x的最小值返回其下标即可
 */
public class LC_528_按权重随机选择 {
    ...
    public int pickIndex() {
        ...
        //一定要加一，因为random.nextInt(10)最大只能得到9,要得到随机数10得加一
